...
RunsModel.prototype.getLatestCompleteRun = function() {
  return when.promise(function(resolve, reject, notify) {
    dbHelper.openDb(function(err, dbConnection) {
      ...
      dbConnection.query('SELECT * FROM runs WHERE status = ? ORDER BY ' +
			'end_time DESC', ['successful'], function(err, result) {
...
RunsModel.prototype.getPreviousScoreAverages = function(numOfDays) {
  return when.promise(function(resolve, reject, notify) {
    dbHelper.openDb(function(err, dbConnection) {
      ...
      dbConnection.query('SELECT run_id, end_time, mean_score, median_score ' +
			'FROM runs WHERE status = ? AND end_time > DATE_SUB(now(), INTERVAL ? ' +
			'DAY) ORDER BY end_time DESC',
